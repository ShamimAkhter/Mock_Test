...
@Component({
  selector: 'app-videojs-recorder-audio-only',
  templateUrl: './videojs-recorder-audio-only.component.html',
  styleUrls: ['./videojs-recorder-audio-only.component.scss']
})
export class VideojsRecorderAudioOnlyComponent implements OnInit,OnDestroy {
  ...
  constructor(elementRef: ElementRef, public storeButton: ButtonEventsStore, public store: CandidateExamStore) {
    this.player = false;
  ...
  ngAfterViewInit() {
    console.log(Wavesurfer); // without this TS removes the unused variable Wavesurfer
    ...
    this.buttonStore = this.storeButton.state$.subscribe({
      next: val => {
        ...
        // console.log(val.recording + "here here");
